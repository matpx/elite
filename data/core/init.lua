...
function core.temp_filename(ext)
    ...
    return temp_dir .. PATHSEP .. temp_file_prefix .. string.format("%06x", temp_file_counter) .. (ext or "")
end
...
function core.load_project_module(force)
    ...
    if system.get_file_info(filename) then
        if not force and not config.auto_load_project_module then
            core.log(
                "untrusted project module found; run core:load-project-module or enable config.auto_load_project_module"
            )
            return true
        end
...
function core.step()
    ...
    local title = (name ~= "---") and (name .. " - elite") or "elite"
...
function core.run()
    while true do
        ...
        local should_idle = config.idle_timeout > 0
            and (not system.window_has_focus() or core.last_input_time + config.idle_timeout < system.get_time())
