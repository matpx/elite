...
local function draw_text(x, y, color)
    ...
    x = renderer.draw_text(style.big_font, "elite", x, y + (dh - th) / 2, color)
    x = x + style.padding.x
...
function Node:get_divider_overlapping_point(px, py)
    if self.type ~= "leaf" then
        ...
        return self.a:get_divider_overlapping_point(px, py) or self.b:get_divider_overlapping_point(px, py)
    end
end
